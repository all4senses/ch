 ...
 */
/*
function ch_preprocess_user_profile(&$profile) {
  //dpm($profile);
}
*/
 ...
 */
function ch_link($variables) {
  
  if (strpos($variables['path'], '://') && !strpos($variables['path'], 'ttp://cloudhostinghq.com')) {
 ...
 */
function ch_preprocess_user_picture(&$picture) {
  ...
  if (!empty($picture['account']->realname)) {
    ...
    $userExtendedData = ch_misc_loadUserExtendedData($picture['account']->uid);
 ...
 * @ingroup themeable
 */
function ch_pager_next($variables) {
  ...
  if ($out) {
    ...
    if(preg_match('|.*href="([^"]*)" .*title="([^"]*)" .*|', $out_altered, $matches) && !empty($matches[1])) {
      ...
      if ($matches[2] == 'Go to next page') {
        ch_misc_addMetatag('next', NULL, $href = 'http://cloudhostinghq.com' . $matches[1]);
      }
    }
  }
  return $out;
 ...
 * @ingroup themeable
 */
function ch_pager_previous($variables) {
  ...
  if ($out) {
    ...
    if(preg_match('|.*href="([^"]*)" .*title="([^"]*)" .*|', $out_altered, $matches) && !empty($matches[1])) {
      ...
      if ($matches[2] == 'Go to previous page') {
        ch_misc_addMetatag('prev', NULL, $href = 'http://cloudhostinghq.com' . $matches[1]);
      }
...
//  }
//  elseif ( $arg_0 == 'node' && (in_array($arg_1, $altered_pager_reviews) || ch_misc_identifyNodeType($arg_1) == 'provider') ) {
 ...
 */
function ch_preprocess_search_results(&$variables) {
...
//  // v1
//  //doesn't work correctly - it create bad links like http://cloudhostinghq.com/search/node/sip%20trunking?page=0%2C0%2C0%2C0%2C0%2C0%2C0%2C1
//  $variables['pager'] = theme('pager', array('tags' => NULL, 'element' => 7));
  ...
  // need prior actions (hack) in function node_search_execute() at node.module
  // v2
  global $ch_num_rows, $ch_limit_rows;
//  dpm('$ch_num_rows =' . $ch_num_rows, '$ch_limit_rows = ' . $ch_limit_rows);
//  dpm($_SESSION['ch_node_search_data']);
//  $page = pager_default_initialize($_SESSION['ch_node_search_data']['ch_num_rows'], $_SESSION['ch_node_search_data']['ch_limit_rows']);
  $page = pager_default_initialize($ch_num_rows, $ch_limit_rows);
 ...
 */
function ch_html_head_alter(&$head_elements) {
...
//  dpm($c_title);
  ...
  if (!empty($head_elements['next']) || !empty($head_elements['prev'])) {
    ...
    if ($current_page > 1) {
      ...
      $current_title = str_replace(' | cloudhostinghq.com', '', $current_title);

      //dpm($current_title);

      global $altered_head_title;
      
      $altered_head_title = $current_title . ' - Page ' . $current_page . ' | Cloud Hosting HQ';
...
//      global $user;
//      if ($user->uid == 1) {
//        ch_misc_addMetatag('title', $altered_head_title);
//      }
    }
  }
 ...
 */
function ch_menu_local_tasks(&$variables) {
  $output = '';
...
/*
function ch_preprocess_breadcrumb(&$variables) {
  dpm($variables);
}
*/
 ...
 */
function ch_breadcrumb($variables) {
 ...
 */
function ch_process_page(&$variables) {
  ...
  // Set breadcrumb
  ch_misc_setBreadcrumbs($variables);
  ...
  ch_misc_addMetatag('stylesheet', NULL, 'http://fonts.googleapis.com/css?family=Open+Sans:normal,bold|Ubuntu:500|Roboto:400,500', 'NA', 'text/css');
 ...
 */
function ch_preprocess_region(&$variables, $hook) {
 ...
 */
function ch_preprocess_block(&$variables, $hook) {
 ...
 *   The name of the template being rendered ("block" in this case.)
 */
function ch_process_block(&$variables, $hook) {
 ...
 */
function ch_preprocess_search_block_form(&$vars) {
 ...
 */
function ch_form_alter(&$form, &$form_state, $form_id) {
...
*/
function ch_preprocess_views_view_rss(&$vars) {
...
*/
function ch_preprocess_views_view_archive(&$vars) {
  dpr('xxxxzzzz');
  exit;
  //dpm($vars['view']);
}
...
* Default theme function for all RSS rows.
*/
function ch_preprocess_views_view_row(&$vars) {
  dpm('xxxxxx');
  exit;  
}
...
* Default theme function for all RSS rows.
*/
function ch_preprocess_views_view_row_rss(&$vars) {
  ...
  if (isset($node->body['und'][0]['value'])) {
    $rss_teaser = ch_misc_getArticleTeaserData('all', $node->body['und'][0]['value'], $vars['row']->nid, 400, TRUE);
    ...
    $rss_teaser = preg_replace('|href="/|', 'href="http://cloudhostinghq.com/', $rss_teaser);
  ...
  if (!empty($extra_data['guest_author'])) {
    ...
    $vars['item_elements'] = preg_replace('|<dc:creator>.*</dc:creator>|', '<dc:creator>' . ch_misc_getUserRealName($node->uid) . '</dc:creator>', $vars['item_elements']);
  }
...
function ch_preprocess_views_view_unformatted(&$vars) {
//  dpr('yyy');
//exit;
...
function ch_preprocess_views_view(&$vars) {
  //dpm($vars);
  //$vars['view']->rows = 'xxx';
  //$vars['rows'] = 'xxx';
  ...
  if ($vars['view']->current_display == 'block_blog_summary') {
    ...
    foreach ($vars['view']->result as $item) {
      ...
      //$url = str_replace('cloudhostinghq.com', 'test.cloudhostinghq.com', $url);
  ...
  /*
  // I set title for preface (at ch_misc_views_pre_render(&$view)) instead of a view itself.
...
//  }
  ...
  if (isset($vars['view']->name) && ($vars['view']->name == 'blog' || $vars['view']->name == 'news') ) {
    $function = 'ch_preprocess_views_view_unformatted'; 
 ...
 * 3. Read carefully, especially within ch_preprocess_html(), there
 ...
 */
function ch_preprocess_html(&$vars) {
  // Load the media queries styles
  // Remember to rename these files to match the names used here - they are
  // in the CSS directory of your subtheme.
//  $media_queries_css = array(
//    'ch.responsive.style.css',
//    'ch.responsive.gpanels.css'
//  );
//  load_subtheme_media_queries($media_queries_css, 'ch');
...
//  );
//  load_subtheme_ie_styles($ie_files, 'ch');
 ...
 * @see comment.tpl.php
 */
function ch_preprocess_comment(&$variables) {
